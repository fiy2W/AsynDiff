...
## Dataset Format
nnSeq2Seq requires a similar dataset structure as nnU-Net.
...
If you want to use more or less GPU memory, please modify the `batch_size` or `patch_size` in `nnSeq2SeqPlans.json`. Note that, `patch_size` needs to be divisible by 16.
...
- visualization: Some intermediate results to monitor model training.
  - epoch_X.jpg: synthesis images and segmentation at epoch X
    |  |  |  |  |  |  |  |  |  |
    |--|--|--|--|--|--|--|--|--|
    | $x^{(1)}_{t_1}$ | ... | $x^{(M)}_{t_M}$ | $\hat{x}^{(1)}_{0}$ | ... | $\hat{x}^{(M)}_{0}$ | $x^{(1)}_{0}$ | ... | $x^{(M)}_{0}$ |

## Run inference
Remember that the data located in the input folder must have the file endings as the dataset you trained the model on and must adhere to the nnU-Net naming scheme for image files (see [dataset format](dataset_format.md)!)
...
data = np.stack([s1, np.zeros_like(s1), s3, s4], axis=0)

# Set propoerties of the sample
properties = {
    'num_channel': 4,
    'available_channel': [0,2,3],
}

# Run inference
pred = pipe.predict_from_image_volume(data=data, properties=properties)

# Save output
sitk.WriteImage(sitk.GetImageFromArray(pred[1]), 'translate_tgt_1.nii.gz')
```
...
data = np.array(np.stack([s1, np.zeros_like(s1), s3, s4], axis=0), dtype=np.half)
...
# Run inference
pred = pipe.predict_from_image_slice_tensor(data=data_slice, tgt_seq=target_id, properties=properties)
