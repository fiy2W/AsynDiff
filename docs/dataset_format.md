...
An example of the structural results for the BraTS2021 Dataset. This dataset has four input channels: T1 (0000), T1Gd (0001), T2 (0002), and Flair (0003). Note that the `imagesTs` folder is optional and does not have to be present.
...
```json
{
    ...
    "normalization": {
        ...
        "1": "RescaleTo005_995to01Normalization",
        "2": "RescaleTo005_995to01Normalization",
        "3": "RescaleTo005_995to01Normalization"
    },
    "labels": {
        "background": 0,
        "NCR": 1,
...
The `normalization` determine the normalization used by nnSeq2Seq. See [here](intensity_normalization.md) for more information about custom intensity normalization.