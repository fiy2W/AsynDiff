...
from nnseq2seq.paths import nnSeq2Seq_preprocessed, nnSeq2Seq_raw
from nnseq2seq.preprocessing.cropping.cropping import crop_to_nonzero
from nnseq2seq.preprocessing.resampling.default_resampling import compute_new_shape
...
from nnseq2seq.utilities.utils import get_identifiers_from_splitted_dataset_folder, \
...
class DefaultPreprocessor(object):
    ...
    def run_case(self, image_files: List[str], seg_file: Union[str, None], plans_manager: PlansManager,
        ...
        data_properties['missing'] = dataset_json['missing']
    ...
    def run(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
        ...
        assert isdir(join(nnSeq2Seq_raw, dataset_name)), "The requested dataset could not be found in nnSeq2Seq_raw"

        plans_file = join(nnSeq2Seq_preprocessed, dataset_name, plans_identifier + '.json')
        assert isfile(plans_file), "Expected plans file (%s) not found. Run corresponding nnSeq2Seq_plan_experiment " \
                                   "first." % plans_file
        ...
        dataset_json_file = join(nnSeq2Seq_preprocessed, dataset_name, 'dataset.json')
        dataset_json = load_json(dataset_json_file)

        output_directory = join(nnSeq2Seq_preprocessed, dataset_name, configuration_manager.data_identifier)
        ...
        dataset = get_filenames_of_train_images_and_targets(join(nnSeq2Seq_raw, dataset_name), dataset_json)
...
def example_test_case_preprocessing():
    ...
    plans_file = '/home/isensee/drives/gpu_data/nnSeq2Seq_preprocessed/Dataset219_AMOS2022_postChallenge_task2/nnSeq2SeqPlans.json'
    dataset_json_file = '/home/isensee/drives/gpu_data/nnSeq2Seq_preprocessed/Dataset219_AMOS2022_postChallenge_task2/dataset.json'
    input_images = ['/home/isensee/drives/e132-rohdaten/nnSeq2Seqv2/Dataset219_AMOS2022_postChallenge_task2/imagesTr/amos_0600_0000.nii.gz', ]  # if you only have one channel, you still need a list: ['case000_0000.nii.gz']
    ...
    # voila. Now plug data into your prediction function of choice. We of course recommend nnSeq2Seq's default (TODO)
    return data
