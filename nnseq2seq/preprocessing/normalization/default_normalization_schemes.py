...
class ZScoreNormalization(ImageNormalization):
    ...
    def run(self, image: np.ndarray, seg: np.ndarray = None) -> np.ndarray:
        ...
        if self.use_mask_for_norm is not None and self.use_mask_for_norm:
            ...
            # The default nnSeq2Seq sets use_mask_for_norm to True if cropping to the nonzero region substantially
...
class RescaleTo005_995to01Normalization(ImageNormalization):
    leaves_pixels_outside_mask_at_zero_if_use_mask_for_norm_is_true = False
