...
class nnSeq2SeqTrainer(object):
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = get_scheduler(
            name="cosine",
            optimizer=optimizer,
            num_warmup_steps=5*self.num_iterations_per_epoch,
    ...
    def random_select_available_multiseqs(self, properties, min_src=0):
        output_code_src = []
        output_all_tgt_code = []
        for bid in range(len(properties)):
            available_channel = [i for i in properties[bid]['available_channel']]
            if properties[bid]['missing'] == 'random':
                select_num = random.choice([i for i in range(min_src, len(available_channel)+1)])
            else:
                select_num = len(available_channel) - properties[bid]['missing']
            seqs_id = np.sort(random.sample(available_channel, select_num))
            output_code_src.append([1 if i in seqs_id else 0 for i in range(self.num_input_channels)])
            output_all_tgt_code.append([1 if i in available_channel else 0 for i in range(self.num_input_channels)])
            
        output_code_src = torch.from_numpy(np.array(output_code_src))
        output_all_tgt_code = torch.from_numpy(np.array(output_all_tgt_code))
        return output_code_src, output_all_tgt_code
    ...
    def train_step(self, batch_id: int, batch: dict) -> dict:
        ...
        src_code, all_tgt_code = self.random_select_available_multiseqs(properties, min_src=0)

        all_tgt_data = ori_data.to(self.device, non_blocking=True)
        all_tgt_code = all_tgt_code.to(self.device, dtype=all_tgt_data.dtype, non_blocking=True)
        src_code = src_code.to(self.device, dtype=all_tgt_data.dtype, non_blocking=True)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            all_tgt_data = torch.cat([all_tgt_data, target/self.network.num_classes], dim=1)
            all_tgt_code = torch.cat([all_tgt_code, torch.ones_like(all_tgt_code)[:,0:1]], dim=1)
            src_code = torch.cat([src_code, torch.zeros_like(src_code)[:,0:1]], dim=1)
            
            noisy_tgt, pred_tgt, l = self.network(all_tgt_data, src_code, all_tgt_code)
        ...
        if batch_id==0:
            with torch.no_grad():
                vimage = [noisy_tgt[:,i:i+1] for i in range(all_tgt_data.shape[1])]
                vimage += [pred_tgt[:,i:i+1] for i in range(all_tgt_data.shape[1])]
                vimage += [all_tgt_data[:,i:i+1] for i in range(all_tgt_data.shape[1])]
                nrow = len(vimage)
                vimage = torch.cat(vimage, dim=1).reshape(-1,1,*(all_tgt_data.shape[2:]))
                if len(all_tgt_data.shape)==5:
                    vimage = vimage[:, :, vimage.shape[2]//2]
                
                vimage = torch.clamp(vimage, min=0, max=1)
                torchvision.utils.save_image(vimage, vis_save_path, nrow=nrow)
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        src_code, all_tgt_code = self.random_select_available_multiseqs(properties, min_src=1)
        
        all_tgt_data = data.to(self.device, non_blocking=True)
        src_code = src_code.to(self.device, dtype=all_tgt_data.dtype, non_blocking=True)
        all_tgt_code = all_tgt_code.to(self.device, dtype=all_tgt_data.dtype, non_blocking=True)
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            with torch.no_grad():
                ...
                pred_tgt = self.network.infer_diffusion(all_tgt_data, src_code, num_inference_steps=1)

                pred_tgt = pred_tgt * all_tgt_code[(...,) + (None,) * (len(all_tgt_data.shape) - 2)] * (all_tgt_data>1e-9)
                all_tgt_data = all_tgt_data * all_tgt_code[(...,) + (None,) * (len(all_tgt_data.shape) - 2)] * (all_tgt_data>1e-9)
                
                l = nn.MSELoss()(pred_tgt, all_tgt_data)
            
        psnr = torch_PSNR(all_tgt_data, pred_tgt, data_range=1)
